...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" OPERATION="FLOAT2FIXED" ROUNDING="TO_NEAREST" WIDTH_DATA=32 WIDTH_EXP_INPUT=8 WIDTH_EXP_OUTPUT=8 WIDTH_INT=24 WIDTH_MAN_INPUT=23 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=38 aclr clk_en clock dataa result
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" PIPELINE=2 SHIFTDIR="VARIABLE" SHIFTTYPE="LOGICAL" WIDTH=60 WIDTHDIST=6 aclr clk_en clock data direction distance result
//VERSION_BEGIN 18.1 cbx_altbarrel_shift 2018:09:12:13:04:24:SJ cbx_mgl 2018:09:12:13:10:36:SJ  VERSION_END

//synthesis_resources = reg 125 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  fp_to_10_14_fixed_altbarrel_shift_d4h
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[59:0]	sbit_piper1d;
	reg	[59:0]	sbit_piper2d;
	reg	sel_pipec3r1d;
	reg	sel_pipec4r1d;
	reg	sel_pipec5r1d;
	wire  [6:0]  dir_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 60'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[179:120];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 60'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[359:300];
	// synopsys translate_off
	initial
		sel_pipec3r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		pad_w = {32{1'b0}},
		result = sbit_w[419:360],
		sbit_w = {sbit_piper2d, smux_w[299:180], sbit_piper1d, smux_w[119:0], data},
		sel_w = {sel_pipec5r1d, sel_pipec4r1d, sel_pipec3r1d, distance[2:0]},
		smux_w = {((({60{(sel_w[5] & (~ dir_w[5]))}} & {sbit_w[327:300], pad_w[31:0]}) | ({60{(sel_w[5] & dir_w[5])}} & {pad_w[31:0], sbit_w[359:332]})) | ({60{(~ sel_w[5])}} & sbit_w[359:300])), ((({60{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[283:240], pad_w[15:0]}) | ({60{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[299:256]})) | ({60{(~ sel_w[4])}} & sbit_w[299:240])), ((({60{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[231:180], pad_w[7:0]}) | ({60{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[239:188]})) | ({60{(~ sel_w[3])}} & sbit_w[239:180])), ((({60{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[175:120], pad_w[3:0]}) | ({60{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[179:124]})) | ({60{(~ sel_w[2])}} & sbit_w[179:120])), ((({60{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[117:60], pad_w[1:0]}) | ({60{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[119:62]})) | ({60{(~ sel_w[1])}} & sbit_w[119:60])), ((({60{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[58:0], pad_w[0]}) | ({60{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[59:1]})) | ({60{(~ sel_w[0])}} & sbit_w[59:0]))};
endmodule //fp_to_10_14_fixed_altbarrel_shift_d4h

//synthesis_resources = lpm_add_sub 6 lpm_compare 4 reg 306 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  fp_to_10_14_fixed_altfp_convert_13o
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	wire  [59:0]   wire_altbarrel_shift2_result;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[37:0]	integer_result_reg;
	reg	[36:0]	integer_rounded_reg;
	reg	man_or1_reg1;
	reg	man_or2_reg1;
	reg	man_or_reg2;
	reg	man_or_reg3;
	...
	wire  [36:0]   wire_add_1_adder_result;
	wire  [5:0]   wire_add_sub1_result;
	wire  wire_add_sub3_cout;
	wire  [17:0]   wire_add_sub3_result;
	wire  [18:0]   wire_add_sub4_result;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [37:0]  all_zeroes_w;
	wire  barrel_direction_negative;
	wire  [59:0]  barrel_mantissa_input;
	wire  [35:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [37:0]  below_limit_integer;
	...
	wire  [37:0]  exceed_limit_integer;
	...
	wire  [37:0]  infinity_value_w;
	wire  int_or1_w;
	wire  int_or2_w;
	wire  [37:0]  integer_output;
	wire  [36:0]  integer_post_round;
	wire  [36:0]  integer_pre_round;
	wire  [37:0]  integer_result;
	wire  [36:0]  integer_rounded;
	wire  [36:0]  integer_rounded_tmp;
	wire  [36:0]  integer_tmp_output;
	wire  [17:0]  inv_add_1_adder1_w;
	wire  [18:0]  inv_add_1_adder2_w;
	wire  [36:0]  inv_integer;
	wire  [59:0]  lbarrel_shift_result_w;
	wire  [36:0]  lbarrel_shift_w;
	wire  lowest_integer_selector;
	wire  [36:0]  lowest_integer_value;
	wire  [10:0]  man_bus1;
	wire  [11:0]  man_bus2;
	wire  [10:0]  man_or1;
	wire  man_or1_w;
	...
	wire  nan_input_w;
	wire  [37:0]  neg_infi_w;
	wire  [7:0]  padded_exponent_input;
	wire  [37:0]  pos_infi_w;
	...
	wire  sign_input;
	wire  sign_input_w;
	wire  [36:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [21:0]  sticky_bus;
	wire  [21:0]  sticky_or;
	wire  [36:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	fp_to_10_14_fixed_altbarrel_shift_d4h   altbarrel_shift2
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 38'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 37'b0;
	...
	.datab(37'b0000000000000000000000000000000000001),
	.overflow(),
	...
	assign
		...
		barrel_zero_padding_w = {36{1'b0}},
		below_limit_exceeders = ((((denormal_input_w | zero_input_w) | nan_input_w) | below_lower_limit3_reg4) & (~ border_lower_limit_reg4)),
		below_limit_integer = (({38{(~ below_limit_exceeders)}} & integer_output) | ({38{below_limit_exceeders}} & all_zeroes_w)),
		...
		const_bias_value_add_width_res_w = 8'b10010110,
		...
		exceed_limit_integer = (({38{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({38{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({38{(~ sign_input_w)}} & pos_infi_w) | ({38{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[0],
		int_or2_w = man_or1[0],
		...
		integer_rounded = (({37{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({37{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({37{(~ add_1_w)}} & integer_pre_round) | ({37{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({37{(~ sign_input_w)}} & unsigned_integer) | ({37{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub3_result,
		inv_add_1_adder2_w = (({19{(~ wire_add_sub3_cout)}} & inv_integer[36:18]) | ({19{wire_add_sub3_cout}} & wire_add_sub4_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift2_result,
		lbarrel_shift_w = lbarrel_shift_result_w[59:23],
...
endmodule //fp_to_10_14_fixed_altfp_convert_13o
//VALID FILE
...
module fp_to_10_14_fixed (
	...
	fp_to_10_14_fixed_altfp_convert_13o	fp_to_10_14_fixed_altfp_convert_13o_component (
				.aclr (aclr),
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
