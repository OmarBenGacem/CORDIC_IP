...
//altfp_convert CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" OPERATION="FLOAT2FIXED" ROUNDING="TO_NEAREST" WIDTH_DATA=32 WIDTH_EXP_INPUT=8 WIDTH_EXP_OUTPUT=8 WIDTH_INT=1 WIDTH_MAN_INPUT=23 WIDTH_MAN_OUTPUT=23 WIDTH_RESULT=8 aclr clk_en clock dataa result
...
//altbarrel_shift CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone V" PIPELINE=2 SHIFTDIR="VARIABLE" SHIFTTYPE="LOGICAL" WIDTH=30 WIDTHDIST=5 aclr clk_en clock data direction distance result
...
module  convert_8_bit_altbarrel_shift_94h
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	reg	[1:0]	dir_pipe;
	reg	[29:0]	sbit_piper1d;
	reg	[29:0]	sbit_piper2d;
	reg	sel_pipec3r1d;
	reg	sel_pipec4r1d;
	wire  [5:0]  dir_w;
	wire  direction_w;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper1d <= 30'b0;
		else if  (clk_en == 1'b1)   sbit_piper1d <= smux_w[89:60];
	// synopsys translate_off
	initial
		sbit_piper2d = 0;
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) sbit_piper2d <= 30'b0;
		else if  (clk_en == 1'b1)   sbit_piper2d <= smux_w[149:120];
	// synopsys translate_off
	initial
		sel_pipec3r1d = 0;
	// synopsys translate_on
	...
	assign
		...
		pad_w = {16{1'b0}},
		result = sbit_w[179:150],
		sbit_w = {sbit_piper2d, smux_w[119:90], sbit_piper1d, smux_w[59:0], data},
		sel_w = {sel_pipec4r1d, sel_pipec3r1d, distance[2:0]},
		smux_w = {((({30{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[133:120], pad_w[15:0]}) | ({30{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[149:136]})) | ({30{(~ sel_w[4])}} & sbit_w[149:120])), ((({30{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[111:90], pad_w[7:0]}) | ({30{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[119:98]})) | ({30{(~ sel_w[3])}} & sbit_w[119:90])), ((({30{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[85:60], pad_w[3:0]}) | ({30{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[89:64]})) | ({30{(~ sel_w[2])}} & sbit_w[89:60])), ((({30{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[57:30], pad_w[1:0]}) | ({30{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[59:32]})) | ({30{(~ sel_w[1])}} & sbit_w[59:30])), ((({30{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[28:0], pad_w[0]}) | ({30{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[29:1]})) | ({30{(~ sel_w[0])}} & sbit_w[29:0]))};
endmodule //convert_8_bit_altbarrel_shift_94h

//synthesis_resources = lpm_add_sub 6 lpm_compare 4 reg 182 
//synopsys translate_off
`timescale 1 ps / 1 ps
//synopsys translate_on
module  convert_8_bit_altfp_convert_pvn
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	wire  [29:0]   wire_altbarrel_shift2_result;
	reg	[4:0]	added_power2_reg;
	...
	reg	int_or_reg2;
	reg	int_or_reg3;
	reg	[7:0]	integer_result_reg;
	reg	[6:0]	integer_rounded_reg;
	reg	man_or1_reg1;
	reg	man_or2_reg1;
	reg	man_or_reg2;
	reg	man_or_reg3;
	...
	reg	max_shift_reg;
	reg	[4:0]	power2_value_reg;
	reg	sign_input_reg1;
	reg	sign_input_reg2;
	reg	sign_input_reg3;
	reg	sign_input_reg4;
	wire  wire_add_1_adder_cout;
	wire  [6:0]   wire_add_1_adder_result;
	wire  [4:0]   wire_add_sub1_result;
	wire  wire_add_sub3_cout;
	wire  [2:0]   wire_add_sub3_result;
	wire  [3:0]   wire_add_sub4_result;
	wire  [4:0]   wire_barrel_direction_invert_result;
	...
	wire  add_1_cout_w;
	wire  add_1_w;
	wire  [7:0]  all_zeroes_w;
	wire  barrel_direction_negative;
	wire  [29:0]  barrel_mantissa_input;
	wire  [5:0]  barrel_zero_padding_w;
	wire  below_limit_exceeders;
	wire  [7:0]  below_limit_integer;
	wire  [7:0]  below_lower_limit3_anding;
	wire  [2:0]  below_lower_limit3_oring;
	...
	wire  [7:0]  infinity_value_w;
	wire  int_or1_w;
	wire  [7:0]  integer_output;
	wire  [6:0]  integer_post_round;
	wire  [6:0]  integer_pre_round;
	wire  [7:0]  integer_result;
	wire  [6:0]  integer_rounded;
	wire  [6:0]  integer_rounded_tmp;
	wire  [6:0]  integer_tmp_output;
	wire  [2:0]  inv_add_1_adder1_w;
	wire  [3:0]  inv_add_1_adder2_w;
	wire  [6:0]  inv_integer;
	wire  [29:0]  lbarrel_shift_result_w;
	wire  [6:0]  lbarrel_shift_w;
	wire  lowest_integer_selector;
	wire  [6:0]  lowest_integer_value;
	wire  [10:0]  man_bus1;
	wire  [11:0]  man_bus2;
	wire  [10:0]  man_or1;
	wire  man_or1_w;
	...
	wire  [4:0]  power2_value_w;
	wire  [7:0]  result_w;
	wire  round_bit_w;
	wire  [7:0]  shift_value_w;
	wire  sign_input;
	wire  sign_input_w;
	wire  [6:0]  signed_integer;
	wire  sticky_bit_w;
	wire  [21:0]  sticky_bus;
	wire  [21:0]  sticky_or;
	wire  [6:0]  unsigned_integer;
	wire  upper_limit_w;
	wire  zero_input_w;

	convert_8_bit_altbarrel_shift_94h   altbarrel_shift2
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	.distance((({5{barrel_direction_negative}} & wire_barrel_direction_invert_result) | ({5{(~ barrel_direction_negative)}} & power2_value_reg))),
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) added_power2_reg <= 5'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_result_reg <= 8'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) integer_rounded_reg <= 7'b0;
	...
	// synopsys translate_on
	always @ ( posedge clock or  posedge aclr)
		if (aclr == 1'b1) power2_value_reg <= 5'b0;
	...
	defparam
		...
		barrel_direction_invert.lpm_width = 5,
	...
	assign
		...
		barrel_zero_padding_w = {6{1'b0}},
		below_limit_exceeders = ((((denormal_input_w | zero_input_w) | nan_input_w) | below_lower_limit3_reg4) & (~ border_lower_limit_reg4)),
		below_limit_integer = (({8{(~ below_limit_exceeders)}} & integer_output) | ({8{below_limit_exceeders}} & all_zeroes_w)),
		...
		below_lower_limit3_oring = {(below_lower_limit3_oring[1] | wire_power2_value_result[7]), (below_lower_limit3_oring[0] | wire_power2_value_result[6]), wire_power2_value_result[5]},
		below_lower_limit3_w = (((wire_power2_value_result[7] & below_lower_limit3_oring[2]) | power2_value_overflow_w) & (~ below_lower_limit3_anding[7])),
		...
		exceed_limit_integer = (({8{(~ exceed_limit_exceeders)}} & below_limit_integer) | ({8{exceed_limit_exceeders}} & infinity_value_w)),
		...
		infinity_value_w = (({8{(~ sign_input_w)}} & pos_infi_w) | ({8{sign_input_w}} & neg_infi_w)),
		int_or1_w = man_or2[5],
		...
		integer_rounded = (({7{(~ lowest_integer_selector)}} & integer_rounded_tmp) | ({7{lowest_integer_selector}} & lowest_integer_value)),
		integer_rounded_tmp = (({7{(~ add_1_w)}} & integer_pre_round) | ({7{add_1_w}} & integer_post_round)),
		integer_tmp_output = (({7{(~ sign_input_w)}} & unsigned_integer) | ({7{sign_input_w}} & signed_integer)),
		inv_add_1_adder1_w = wire_add_sub3_result,
		inv_add_1_adder2_w = (({4{(~ wire_add_sub3_cout)}} & inv_integer[6:3]) | ({4{wire_add_sub3_cout}} & wire_add_sub4_result)),
		inv_integer = (~ integer_rounded_reg),
		lbarrel_shift_result_w = wire_altbarrel_shift2_result,
		lbarrel_shift_w = lbarrel_shift_result_w[29:23],
		...
		neg_infi_w = {1'b1, {7{1'b0}}},
		padded_exponent_input = exponent_input,
		pos_infi_w = {1'b0, {7{1'b1}}},
		power2_value_overflow_w = wire_power2_value_overflow,
		power2_value_w = wire_power2_value_result[4:0],
		result = result_w,
...
endmodule //convert_8_bit_altfp_convert_pvn
//VALID FILE
...
module convert_8_bit (
	...
	convert_8_bit_altfp_convert_pvn	convert_8_bit_altfp_convert_pvn_component (
				.aclr (aclr),
				.clk_en (clk_en),
				.clock (clock),
				.dataa (dataa),
